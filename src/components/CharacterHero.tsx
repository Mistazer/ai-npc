    ...
    <section className="surface relative mb-6 overflow-hidden">
      ...
      {card.splash ? (
        <img
          ...
          alt=""
          aria-hidden
          className="pointer-events-none absolute right-0 top-0 h-full w-1/2 object-cover opacity-25 [mask-image:linear-gradient(to_left,black,transparent)]"
        />
      ) : null}
      ...
      <div className="relative flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:p-6">
        <img
          src={card.icon ?? ""}
          alt={card.name}
          className="h-24 w-24 shrink-0 rounded-2xl border-2 object-cover sm:h-28 sm:w-28"
          style={{ borderColor: card.rarityRank === 5 ? "var(--r-5)" : "var(--r-4)" }}
        />
